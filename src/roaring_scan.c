 ...
 * roaring_getbitmap
 *
 * Single-column: raw int8/int4 value lookup.
 * Multi-column: one bitmap per scan key (index + pending),
...
roaring_gettuple(IndexScanDesc scan, ScanDirection dir)
{
	...
	if (scan->xs_want_itup)
	{
		...
		/*
		 * A key column's value is known only from a plain equality scan key.
		 * IS NULL columns stay NULL; columns constrained only by IN / IS NOT
		 * NULL, or not constrained at all, also stay NULL here — an IndexOnly
		 * scan cannot reconstruct them.  The planner steers any query that
		 * projects such an unconstrained key column to a bitmap/heap scan
		 * instead (see roaring_suppress_unreturnable_ios), so a NULL here is
		 * only ever produced for columns the query does not actually read.
		 */
		for (i = 0; i < scan->numberOfKeys; i++)
		{
